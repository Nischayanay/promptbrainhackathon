...
import { ChevronDown, AudioWaveform, Send, Sparkles, Copy, Download } from 'lucide-react';
...
import { createBackendBrainService } from '../../backend-brain/services/backend-brain-service';
...
export function SearchInterface() {
  const { user } = useAuth();
  const { balance: credits, deductCredits } = useCredits();
  ...
  const [enhancementRatio, setEnhancementRatio] = useState<number>(0);
  ...
  const handleSend = async () => {
    ...
    if (!user) {
      setValidationMessage('🔐 Please log in to enhance prompts');
      return;
    }

    if (credits <= 0) {
      setValidationMessage('💳 No credits remaining. Credits refresh daily.');
      return;
    }
    ...
    try {
      // Call Backend Brain service
      const result = await backendBrainService.enhancePrompt(value.trim(), user.id);
      
      setOutput(result.enhancedText);
      setEnhancedJson(result.enhancedJson);
      setQualityScore(result.qualityScore);
      setEnhancementRatio(result.metadata.enhancementRatio);
      
      // Deduct credits
      await deductCredits(1, 'Backend Brain prompt enhancement');
      
      // Show success affirmation
      const affirmation = affirmations[Math.floor(Math.random() * affirmations.length)];
      toast.success(affirmation.message, {
        description: `Quality: ${(result.qualityScore * 100).toFixed(0)}% • ${result.metadata.enhancementRatio.toFixed(1)}x enhanced`,
        duration: 4000
      });
      ...
      setValidationMessage(`❌ Enhancement failed: ${error.message || 'Please try again'}`);
            ...
            >
              ...
              >
                <div className="flex items-center" style={{ gap: 'var(--spacing-1)' }}>
                  ...
                  {qualityScore > 0 && (
                    <span className="text-[#00D9FF]" style={{ fontSize: 'var(--text-xs)' }}>
                      • {(qualityScore * 100).toFixed(0)}% quality • {enhancementRatio.toFixed(1)}x enhanced
                    </span>
                  )}
                </div>
                ...
                <div className="flex items-center" style={{ gap: 'var(--spacing-1)' }}>
                  <motion.button
                    ...
                    className="glass-card rounded-lg text-white/70 hover:text-white hover:bg-white/5"
                    style={{
                  ...
                  {enhancedJson && (
                    <motion.button
                      onClick={() => {
                        const blob = new Blob([JSON.stringify(enhancedJson, null, 2)], { type: 'application/json' });
                        ...
                        toast.success('JSON downloaded!', { duration: 2000 });
                      }}
                      className="glass-card rounded-lg text-white/70 hover:text-white hover:bg-white/5"
                      style={{
                        padding: 'var(--spacing-1-5)',
